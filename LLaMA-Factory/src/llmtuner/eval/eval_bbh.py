...
def eval_bbh(model, tokenizer, dataset, batch_size=1, output_result_dir=None, use_prompt=False):
    ...
    for sample in dataset:
        ...
        few_prompt_list = []
        for d in task_prompt.split('Q:'):
            few_prompt_list.extend(d.split('A:'))
        messages = []
        for ind, d in enumerate(few_prompt_list):
            if ind == 0:
                continue
            if ind == 1:
                messages.append({"role": "user", "content": few_prompt_list[0].strip()+'\nQ: '+d.strip()})
            elif ind % 2 == 0 :# 2,4,6
                messages.append({"role": "assistant", "content": 'A: '+d.strip()})
            else: # 3,5
                messages.append({"role": "user", "content": 'Q: '+d.strip()})
        prompt = "Following previous examples, answer the following questions and end with 'So the answer is <your answer here>'.\n Q: " + \
                 question
        messages.append({"role": "user", "content": prompt})
        prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        prompt += "A: Let's think step by step." if prompt[-1] in ["\n", " "] else " A: Let's think step by step."
        # set_trace()
