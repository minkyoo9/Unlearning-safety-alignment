...
class IterativeUnlearner(Trainer):
    ...
    def get_batch_loss(self, logits, labels):
        ...
        Args:
            logits: 모델의 출력 로짓 텐서 ([batch_size, seq_len, vocab_size]).
            labels: 실제 레이블 텐서 ([batch_size, seq_len]).

        Returns:
            각 시퀀스에 대한 손실이 담긴 텐서 ([batch_size]).
        """
        # 다음 토큰을 예측하도록 시프트
        ...
        # 각 토큰별 손실 계산
        loss = loss_function(shifted_logits.view(-1, shifted_logits.size(-1)), shifted_labels.view(-1))
        # 손실을 [batch_size, seq_len - 1]로 reshape하고 시퀀스 길이 방향으로 합산
        loss = loss.view(shifted_labels.size(0), shifted_labels.size(1)).sum(dim=-1)
    ...
    def compute_loss(self, model, x, return_outputs=False):
        ...
        if 'ga' in self.loss_type:
            ...
            # 현재 모델의 각 시퀀스별 손실 계산
            forget_loss_current = self.get_batch_loss(outputs_f.logits, x_f['labels'])
            # 참조 모델의 각 시퀀스별 손실 계산
            forget_loss_ref = self.get_batch_loss(outputs_f_ref.logits, x_f['labels'])
            # 음의 로그 가능도 차이 계산
            neg_log_ratio = forget_loss_current - forget_loss_ref
            # NPO 손실 계산
